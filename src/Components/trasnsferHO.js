...
import { TransferHOreq, TransferSalesreq, isAuthenticated } from "../auth/authIndex";
...
export default function TransferHO(){
  var nav = useNavigate();
    ...
    const handleChange = (selectedOption) => {
        ...
        setdshow(dist.filter(ele=> ele.value !== selectedOption.salesid))
        reset();
      };
const handleChangedist = (selectedOption) => {
  ...
  const SendUpdate = ()=>{
    var resp = TransferHOreq({id:values.sales,salesid:senddist.dist_id,email:values.email,role_id:values.role_id});
    if (resp){
    ...
    return(
      ...
      <Row>
        ...
        {sales && <div className="SalesSelect">
          <Form.Label style={{fontWeight:"600"}}>Select Head Office :</Form.Label>
        <Select
        ...
        <Row>
          <Col xs={10}>
        {values.dist && <>
          <Form.Label style={{fontWeight:"600",marginTop:"6vh"}}>Select Sales :</Form.Label>
        <Select
        key={seed}
        className="basic-single"
        classNamePrefix="select"
