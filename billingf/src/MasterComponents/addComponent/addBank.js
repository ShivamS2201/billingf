  ...
  const handleChange = (name) => (event) => {
    if (name === "gstNumber") { // checks will be sequential 1. State Code and 1 st 2 letters same.
      ...
      if (event.target.value.slice(0,2)<10){
        if (values.StateCode && (event.target.value.slice(1,2) === values.StateCode)){
          console.log(event.target.value.slice(1,2),values.StateCode,"s")
          if (resp){
            setvalues({
              ...values,
              error: false,
        ...
        if (values.StateCode && (event.target.value.slice(0,2) === values.StateCode)){
    ...
    return (
      <Form.Group>
        ...
        <Form.Select
          ...
          required
        >
          {JSON.stringify(values.StateCode)}
          <option defaultValue>Open this select State</option>
    ...
    return (
      <Form.Group>
        ...
        <Form.Select
          ...
          required
        >
          {JSON.stringify(values.account_type)}
